...
function searchLocation(city, state, searchTerm) {
    return postgrePool.query('SELECT * FROM nearbyplaces.places WHERE city=$1 AND state=$2 AND (%$3% like "name" OR %$3% like description)', [city, state, searchTerm]).then(x => x.rows);
}
